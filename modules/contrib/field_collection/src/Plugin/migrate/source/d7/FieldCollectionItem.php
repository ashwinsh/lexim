...
class FieldCollectionItem extends FieldableEntity {
  ...
  public function query() {
    ...
    if (isset($this->configuration['field_name'])) {
      ...
      //If multilingual, get language from node.
      if (isset($this->configuration['translations'])) {
        ...
        $query->fields('n', array('tnid', 'language'));
      }
  ...
  public function prepareRow(Row $row) {
    ...
    //If multilingual, set the same item ID for the item throughout its translations.
    if (isset($this->configuration['translations'])) {
      $tnid = $row->getSourceProperty('tnid');
      $delta = $row->getSourceProperty('delta');
      $row->setSourceProperty('item_id', \Drupal::service('field_collection.migration_translation_transform')->setTranslationItemID($tnid, $delta));
    }
